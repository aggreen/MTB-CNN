...
# coding: utf-8
"""
Runs multitask model with conv-pool architecture, 5 fold cross validation on training/validation set
Note that this architecture was NOT SELECTED for the final model
Authors:
...
def run():
    ...
    # Get just the X and Y entries with phenotypes
    ...
    ### Run cross validation and assessment
    cv_splits = 5
    ...
    for train_idx, (train, val) in enumerate(cv.split(X, y)):
        ...
        y_train = y[train, :]
        y_val = y[val, :]
        print("shape of x input", X_train.shape)
        print('fitting..')
        history = model.fit_model(X_train, alpha_matrix[train, :],
                                  X_val, alpha_matrix[val, :])
