...
def compute_drug_auc_table(y_train, y_train_pred, drug_to_threshold, DRUG):
	...
	for idx, drug in enumerate([DRUG]):
		...
		val_threshold = float(drug_to_threshold)

		non_missing_val = np.where(y_train[:, idx] != -1)[0]

		print("number of non-missing values", non_missing_val, np.where(y_train[:, idx] != -1)[0].shape)
		print("shape of the y_train data", y_train.shape, y_train_pred.shape)

		if len(non_missing_val)==0:
			results.loc[idx] = ['CNN', drug, 0, 0, np.nan, np.nan, val_threshold, np.nan, np.nan]
			continue

		auc_y = np.reshape(y_train[non_missing_val], (len(non_missing_val))).astype(int)
		auc_preds = np.reshape(y_train_pred[non_missing_val], (len(non_missing_val)))
		...
		y_average_precision = (1 - y_train[non_missing_val]).astype(int)
		pred_average_precision = (1 - y_train_pred[non_missing_val]).astype(int)
		...
		N_condition_negative = np.sum(y_train[non_missing_val] == 0)
		N_condition_positive = np.sum(y_train[non_missing_val] == 1)
		...
		val_spec = np.sum(np.logical_and(binary_prediction == 0, y_train[non_missing_val] == 0)) / N_condition_negative
		# Sensitivity = #TP / # Condition Positive
		val_sens = np.sum(np.logical_and(binary_prediction == 1, y_train[non_missing_val] == 1)) / N_condition_positive
...
def run():
    def compute_y_pred(df_geno_pheno_test, DRUG):

        if len(df_geno_pheno_test)==0:
        ...
        return y_pred.reshape(-1,1), y_test.reshape(-1,1)
    ...
    print(y_train.reshape(-1,1).shape, y_train_pred.reshape(-1,1).shape)
    ...
    results = compute_drug_auc_table(y_train, y_train_pred, threshold, DRUG)
    ...
    y_pred2, y_test2 = compute_y_pred(df_geno_pheno_test, DRUG)

    df_geno_pheno_test = df_geno_pheno.query("category=='set3_matthias_in_rollingdb'")
    y_pred3, y_test3 = compute_y_pred(df_geno_pheno_test, DRUG)

    df_geno_pheno_test = df_geno_pheno.query("category=='set4_matthias_new_vcf'")
    y_pred4, y_test4 = compute_y_pred(df_geno_pheno_test, DRUG)

    print(y_pred2.shape, y_pred3.shape, y_pred4.shape)
    print(y_test2.shape, y_test3.shape, y_test4.shape)

    y_pred = np.concatenate([y_pred2, y_pred3, y_pred4])
    y_test = np.concatenate([y_test2, y_test3, y_test4])

    results = compute_drug_auc_table(y_test, y_pred, threshold, DRUG)
